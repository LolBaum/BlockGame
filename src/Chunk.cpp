        ...
        case 6:
            for (int x = 0; x < CX; x++) {
                for (int y = 0; y < CY; y++) {
                    for (int z = 0; z < CY; z++) {
                        float h = glm::abs(
                                glm::perlin(glm::vec3((x + pos.x) / 16.0, (y + pos.y) / 16.0, (z + pos.z) / 16.0)));
                        //std::cout << "perlin: " << h << std::endl;
                        if (h < 0.2) {
                            blocks[x][y][z].setId(1);
                        }
                        ...
                        switch (bt->get_texture_type()) // ToDo: Get the texture Type from the Blockmanager
                        {
                            ...
                            case SingleTexture: {
                                ...
                                if (bt->get_transparency_type() == Solid) {
                                    ...
                                    if (getBlockType(x - 1, y,
                                                     z)->isTransparent()) { // Todo: check for opeaqeness //TODO: Add explanation for this!!!
                                    ...
                                    mesh_transparent.addPlane_basic_lighting(glm::vec3(x, y, z) + dummyPos, X_negative,
                                                                             tex_x,
                                                                             tex_y, 0);
                                    mesh_transparent.addPlane_basic_lighting(glm::vec3(x, y, z) + dummyPos, X_positive,
                                                                             tex_x,
                                                                             tex_y, 0);
                                    mesh_transparent.addPlane_basic_lighting(glm::vec3(x, y, z) + dummyPos, Y_negative,
                                                                             tex_x,
                                                                             tex_y, 0);
                                    mesh_transparent.addPlane_basic_lighting(glm::vec3(x, y, z) + dummyPos, Y_positive,
                                                                             tex_x,
                                                                             tex_y, 0);
                                    mesh_transparent.addPlane_basic_lighting(glm::vec3(x, y, z) + dummyPos, Z_negative,
                                                                             tex_x,
                                                                             tex_y, 0);
                                    mesh_transparent.addPlane_basic_lighting(glm::vec3(x, y, z) + dummyPos, Z_positive,
                                                                             tex_x,
                                                                             tex_y, 0);
                                } else if (bt->get_transparency_type() == Foliage) {
                                    mesh_foliage.add_cross(glm::vec3(x, y, z) + dummyPos, tex_x, tex_y, 0.9990);
                                    mesh_foliage.add_Block_with_offset(glm::vec3(x, y, z) + dummyPos, tex_x, tex_y,
                                                                       0.90);
                                } else if (bt->get_transparency_type() == Glass) {
                                    if (getBlockType(x - 1, y,
                                                     z)->isTransparent()) { // Todo: check for opeaqeness //TODO: Add explanation for this!!!
                                        mesh_foliage.addPlane_basic_lighting(glm::vec3(x, y, z) + dummyPos, X_negative,
                                                                             tex_x,
                                                                             tex_y, 0);
                                    }
                                    if (getBlockType(x + 1, y, z)->isTransparent()) { // Todo: check for opeaqeness
                                        mesh_foliage.addPlane_basic_lighting(glm::vec3(x, y, z) + dummyPos, X_positive,
                                                                             tex_x,
                                                                             tex_y, 0);
                                    }
                                    if (getBlockType(x, y - 1, z)->isTransparent()) { // Todo: check for opeaqeness
                                        mesh_foliage.addPlane_basic_lighting(glm::vec3(x, y, z) + dummyPos, Y_negative,
                                                                             tex_x,
                                                                             tex_y, 0);
                                    }
                                    if (getBlockType(x, y + 1, z)->isTransparent()) { // Todo: check for opeaqeness
                                        mesh_foliage.addPlane_basic_lighting(glm::vec3(x, y, z) + dummyPos, Y_positive,
                                                                             tex_x,
                                                                             tex_y, 0);
                                    }
                                    if (getBlockType(x, y, z - 1)->isTransparent()) { // Todo: check for opeaqeness
                                        mesh_foliage.addPlane_basic_lighting(glm::vec3(x, y, z) + dummyPos, Z_negative,
                                                                             tex_x,
                                                                             tex_y, 0);
                                    }
                                    if (getBlockType(x, y, z + 1)->isTransparent()) { // Todo: check for opeaqeness
                                        mesh_foliage.addPlane_basic_lighting(glm::vec3(x, y, z) + dummyPos, Z_positive,
                                                                             tex_x,
                                                                             tex_y, 0);
                                    }
                                } else {
                            ...
                            case MultiTexture: {
                                ...
                                if (bt->isRotatable()) {
                                    ...
                                    int face_rotation_X = 0;
                                    int face_rotation_Y = 0;
                                    int face_rotation_Z = 0;
                                    SpecialBlockTexture *tex = bt->get_multi_texture();

                                    std::cout << "update Chunk Multitexture" << std::endl;

                                    const BlockDirection order_z_positive[6] = {X_negative, X_positive, Y_negative, Y_positive,
                                                                     Z_negative, Z_positive};
//                                    const BlockDirection order_z_negative[6] = {Y_positive, X_positive, X_negative, Y_negative,
//                                                                     Z_negative, Z_positive};
                                    const BlockDirection order_y_positive[6] = {X_negative, X_positive, Z_negative, Z_positive,
                                                                                Y_negative, Y_positive};
//                                    const BlockDirection order_y_negative[6] = {Y_positive, X_positive, Z_positive, Z_negative,
//                                                                     X_negative, Y_negative};
                                    const BlockDirection order_x_positive[6] = {Z_negative, Z_positive, Y_negative, Y_positive,
                                                                                X_negative, X_positive};
//                                    const BlockDirection order_x_negative[6] = {Z_positive, Z_negative, X_positive, Y_positive,
//                                                                     Y_negative, X_negative};

                                    std::cout << "0: " << order_z_positive[0] <<  ", 1: " << order_z_positive[1] << ", 2: " << order_z_positive[2] << ", 3: " << order_z_positive[3] << ", 4: " << order_z_positive[4] << ", 5: " << order_z_positive[5] << std::endl;
